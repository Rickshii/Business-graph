...
class GraphService {
    ...
    async addNode(node) {
        ...
        const newNode = { ...node, id, properties: node.properties || {} };
        if (this.isNeo4jConnected && this.driver) {
            const session = this.driver.session();
            try {
                await session.run(`CREATE (n:${newNode.type} {id: $id, label: $label, properties: $properties}) RETURN n`, { id, label: newNode.label, properties: JSON.stringify(newNode.properties) });
            }
            finally {
                await session.close();
            }
    ...
    async updateNode(id, updates) {
        ...
        if (this.isNeo4jConnected && this.driver) {
            const session = this.driver.session();
            try {
                await session.run(`MATCH (n {id: $id}) SET n.label = $label, n.properties = $properties RETURN n`, { id, label: updated.label, properties: JSON.stringify(updated.properties) });
            }
            finally {
                await session.close();
            }
    ...
    async addEdge(edge) {
        ...
        const newEdge = { ...edge, id, properties: edge.properties || {} };
        if (this.isNeo4jConnected && this.driver) {
            const session = this.driver.session();
            try {
                await session.run(`MATCH (s {id: $sourceId}), (t {id: $targetId})
    ...
    async updateEdge(id, updates) {
        ...
        if (this.isNeo4jConnected && this.driver) {
            const session = this.driver.session();
            try {
                await session.run(`MATCH ()-[r {id: $id}]->() SET r.weight = $weight, r.properties = $properties RETURN r`, { id, weight: updated.weight, properties: JSON.stringify(updated.properties) });
            }
            finally {
                await session.close();
            }
    ...
    async runPageRank(iterations = 20, dampingFactor = 0.85) {
    ...
    async runFraudDetection() {
        ...
        const findCycles = (node, visited, path) => {
            visited.add(node);
            path.push(node);
        ...
        const customerReviewMap = {}; // cust -> [businessId]
        const customerBuysMap = {}; // cust -> [businessId]
        edges.forEach(e => {
            if (e.type === 'BUYS_FROM') {
    ...
    async runSupplyChain(sourceId, targetId) {
        const nodes = Array.from(this.mockNodes.keys());
        if (!this.mockNodes.has(sourceId) || !this.mockNodes.has(targetId))
            return [];
        const edges = Array.from(this.mockEdges.values());
        const adj = {};
        nodes.forEach(id => (adj[id] = []));
        ...
        const paths = [];
        const traverse = (current, target, visited, path) => {
            visited.add(current);
            path.push(current);
            if (current === target) {
                paths.push([...path]);
    ...
    async runPartnershipPrediction() {
        const nodes = Array.from(this.mockNodes.values());
        ...
        edges.forEach(e => {
            if (neighbors[e.sourceId])
                neighbors[e.sourceId].add(e.targetId);
            if (neighbors[e.targetId])
                neighbors[e.targetId].add(e.sourceId);
        });
        const predictions = [];
        ...
        for (let i = 0; i < businesses.length; i++) {
            for (let j = i + 1; j < businesses.length; j++) {
                ...
                const directlyConnected = edges.some(e => (e.sourceId === b1 && e.targetId === b2) || (e.sourceId === b2 && e.targetId === b1));
    ...
    async searchNodes(query, type) {
        ...
        return nodes.filter(n => {
            const matchesQuery = !query ||
                ...
                Object.values(n.properties).some(v => String(v).toLowerCase().includes(query.toLowerCase()));
