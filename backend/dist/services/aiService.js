...
class AIService {
    async queryKnowledgeGraph(prompt) {
        ...
        // 2. Analyze for Fraud
            ...
            const fraudData = await graphService_1.graphService.runFraudDetection();
            ...
            Object.keys(fraudData.riskScores).forEach(id => {
                if (fraudData.riskScores[id] > 0.4)
                    flaggedIds.add(id);
            });
        ...
        // 5. Partnership Prediction
            ...
            const preds = await graphService_1.graphService.runPartnershipPrediction();
        ...
        // 6. Generic / Search Nodes
            ...
            matchedNodes = nodes.filter(n => queryWords.some(w => n.label.toLowerCase().includes(w) || n.type.toLowerCase().includes(w)));
